...
<div class="app">
    ...
    <div v-else>
        <div class="general-info">
            <div class="info">
                <div class="payment" v-if="calculator_data.type == 'anuent'">
                    Ежемесячный платеж: {{ displayMoney(Credit.annuityPayment)}}
                </div>
                <div class="payment" v-else>
                    Ежемесячный платеж: Изменяется каждый месяц
                </div>
            ...
            <router-link
                ...
                :to="{
                    ...
                    params: {reset_data: {
                        ...
                        type: this.calculator_data.type
                    }}
                }"
...
export default {
    name: 'CreditCalculatorResultMain',
    components:{
        Chart
    },
    data(){
    ...
    methods: {
        ...
        calculateAnnuity(){
            ...
            this.Credit.annuityPayment = this.calculator_data.amount * (rate + tmp)
            this.Credit.overpayment = this.Credit.annuityPayment * alone - this.calculator_data.amount
            this.Credit.total_amount = this.Credit.annuityPayment * alone
            this.dataToTableAnnuity()
        },  
        calculateDifferentiated(){
            let total = this.calculator_data.amount
            let alone = this.calculator_data.alone * this.calculator_data.size_alone
			let rate = this.calculator_data.rate / 100 / 12
			let principal = total / alone
            var overpayment = 0
            var interest_tmp = 0
			for (let i = 1; i <= alone; i++) {
				interest_tmp = total * rate
                overpayment += interest_tmp
                total -= principal
			}
            this.Credit.overpayment = overpayment
            this.Credit.total_amount = this.calculator_data.amount + overpayment
            this.dataToTableDifferentiated()
        },
        dataToTableDifferentiated(){
            let total = this.calculator_data.amount
            let alone = this.calculator_data.alone * this.calculator_data.size_alone
			let rate = this.calculator_data.rate / 100 / 12
			let principal = total / alone
    ...
    mounted(){
        if (this.calculator_data.type == 'anuent')
            this.calculateAnnuity()
        else
            this.calculateDifferentiated()
        let credit = this.calculator_data.amount
